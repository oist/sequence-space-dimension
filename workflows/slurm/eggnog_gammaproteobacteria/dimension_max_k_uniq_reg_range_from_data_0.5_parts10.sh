...
#SBATCH --output=/flash/KondrashovU/ladaisa/logs/eggnoggamma/pdistm_logs/dim_reg_range_%A_%a.log
...
while read -r INFILE; do 
	python3 /bucket/KondrashovU/seq_space/scripts/dimension_steepest_curve_from_matrix.py -f $INFILE -i $INP/pdistm_matrices_unique/ -of $OUT/kcoefs_mult_ali_max_k_range_from_data/ -op $OUT/kcoefs_mult_ali_max_k_range_from_data_plots/ -w 0.5 -n 20 -r True -p True -s True -m 'max' -l False
